...
def genere_strat(carre, move, nb):
    st='board.getMarbleCount(self.player) + '+str(carre)+'*acte_moi + '+str(move)+'*move_moi_utile + '+str(nb)+'*nbcarretotal - board.getMarbleCount(-self.player) - '+str(carre)+'*acte_adv - '+str(move)+'*move_adv_utile'
    return str(st)
...
f = open("acte_vs_s0_prof4.pkl","wb")
